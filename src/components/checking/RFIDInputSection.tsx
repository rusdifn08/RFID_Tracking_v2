...
const RFIDInputSection = memo(({ 
    ...
    const placeholder = rfidPlaceholder ?? 'Scan atau ketik RFID untuk checking...';
    // Hitung statistik
    const stats = useMemo(() => {
    ...
    return (
        <div className={`bg-white border-2 ${t.border} rounded-lg p-3 xs:p-4 sm:p-5 md:p-6 hover:shadow-lg ${t.borderHover} transition-all duration-300`}>
            ...
            <div className="flex flex-col lg:flex-row gap-3 xs:gap-4 sm:gap-5">
                ...
                <div className="flex-1 lg:flex-none lg:w-2/3 xl:w-3/5">
                    <div className="flex flex-col xs:flex-row items-stretch xs:items-center gap-2 xs:gap-3">
                        <div className="flex-1 relative">
                            <Search className={`absolute left-2 xs:left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 xs:w-4.5 sm:w-5 h-4 xs:h-4.5 sm:h-5 ${t.icon}`} />
                            <input
                                ref={inputRef}
                                type="text"
                                value={rfidInput}
                                ...
                                disabled={isChecking}
                                className={`w-full pl-8 xs:pl-10 sm:pl-12 pr-3 xs:pr-4 py-2 xs:py-2.5 sm:py-3 bg-white border-2 ${t.border} rounded-lg text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 ${t.inputFocus} transition-all duration-200 font-mono text-sm xs:text-base ${t.inputHover}`}
                            />
                        </div>
                        <button
                            onClick={onCheck}
                            disabled={isChecking || !rfidInput.trim()}
                            className={`px-4 xs:px-5 sm:px-6 py-2 xs:py-2.5 sm:py-3 ${t.button} disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-bold rounded-lg transition-all duration-200 flex items-center gap-1.5 xs:gap-2 justify-center text-sm xs:text-base whitespace-nowrap`}
                        >
                            {isChecking ? (
                                <>
                ...
                <div className="flex-1 lg:flex-none lg:w-1/3 xl:w-2/5">
                    <div className="grid grid-cols-3 gap-2 xs:gap-2.5 sm:gap-3 h-full">
                        {/* Total Checks */}
                        <div className={`${t.statTotalBg} border ${t.statTotalBorder} rounded-lg p-2 xs:p-2.5 sm:p-3 flex flex-col items-center justify-center`}>
                            <BarChart3 className={`w-4 xs:w-5 sm:w-6 h-4 xs:h-5 sm:h-6 ${t.icon} mb-1 xs:mb-1.5`} />
                            ...
                            <p className={`text-base xs:text-lg sm:text-xl md:text-2xl font-bold ${t.statTotalText} leading-none`}>
                                {stats.total}
                            </p>
                        </div>
