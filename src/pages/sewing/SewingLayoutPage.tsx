  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (payload.data.length === 0) {
        throw new Error('Assign minimal 1 operator sebelum simpan layout.');
      }
      ...
      <div className="mx-auto w-full max-w-[min(100%,72rem)] px-[clamp(0.5rem,2vw,1.25rem)] py-[clamp(0.5rem,2vh,1rem)]">
        <section
          className={cn(
            ...
            'before:pointer-events-none before:absolute before:inset-0 before:bg-gradient-to-br before:from-violet-500/10 before:to-blue-500/5'
          )}
        >
          <div className="relative">
            <div className={cn('mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold', CHIP.purple)}>
              <LayoutGrid className="h-3.5 w-3.5" />
              Daftar Layout • Line {normalizedLine}
            </div>
            <h1 className="m-0 text-[clamp(1.35rem,3vw,2.1rem)] font-extrabold leading-tight tracking-tight">
              Gabungkan SMV & Operator
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-500">
              Assign operator sewing ke setiap proses SMV. Simpan akan menulis plotting ke JSON lokal (
              <code className="text-xs">sewing_layout_post.json</code>) dan mencoba POST{' '}
              <code className="text-xs">/api/smv/master/combine</code> dengan body{' '}
              <code className="text-xs">smv_id</code>, <code className="text-xs">rfid_user</code>,{' '}
              <code className="text-xs">batch</code>.
            </p>
            ...
            <div className="mt-4 flex flex-wrap items-end gap-3">
              <label className="flex min-w-[10rem] flex-col gap-1 text-xs font-semibold text-slate-600">
                ...
                <input
                  ...
                  className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-800 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
                  placeholder="G81449"
                />
              </label>
              <button
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-violet-700 px-4 py-2 text-sm font-semibold text-white shadow-md transition hover:from-violet-700 hover:to-violet-800 disabled:opacity-60"
              >
            ...
            <table className="w-full min-w-[56rem] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/90">
                  ...
                  <th className={cn(thClass, 'min-w-[12rem]')}>Proses</th>
              ...
              <tbody>
                ...
                {!loading && displaySmvRows.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-10 text-center text-slate-500">
                      Tidak ada data SMV untuk style ini.
                    </td>
                  </tr>
                ) : null}
                {displaySmvRows.map((row) => {
                  ...
                  return (
                    <tr key={row.smv_id} className="border-b border-slate-100 hover:bg-violet-50/30">
                      ...
                      <td className="max-w-[18rem] px-3 py-2.5 text-slate-800">{row.nama_proses}</td>
                      ...
                      <td className="px-3 py-2.5">
                        <select
                          ...
                          className="w-full min-w-[10rem] rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
                        >
...
SewingLayoutPage.displayName = 'SewingLayoutPage';

export default SewingLayoutPage;
